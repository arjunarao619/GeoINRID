...
def compute_embeddings(loc_enc, coords_deg: np.ndarray, device='cuda') -> np.ndarray:
    # GridCellSpatialRelationEncoder expects degrees
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Local MLE ID density analysis for CSP location encoders."
    )
...
def main():
    ...
    wandb.init(
        ...
        name="local_mle_density_analysis_csp"
    )
    ...
    # Define your CSP model checkpoints
    encoders = {
        "csp_gridcell_inat2018": {
            "checkpoint": "/projects/arra4944/csp/csp/weights/model_dir/model_inat_2018/model_inat_2018_gridcell_0.0010_32_0.1000000_1_512_leakyrelu_UNSUPER-contsoftmax_0.000500_1.000_1_1.000_TMP20.0000_1.0000_1.0000.pth.tar",
        },
        "csp_gridcell_sop": {
            "checkpoint": "/projects/arra4944/csp/csp/weights/model_dir/model_fmow/model_fmow_gridcell_0.0010_32_0.1000000_1_512_gelu_UNSUPER-contsoftmax_0.000050_1.000_1_0.100_TMP1.0000_1.0000_1.0000.pth.tar",
        },
        # Add more CSP variants as needed
    }
    
    for sampling in args.sampling:
        sampler = get_sampler(sampling)
        coords_deg = sampler(args.n)  # (lon,lat) in degrees
        ...
        for name, meta in encoders.items():
            ...
            # Load model
            try:
                loc_enc = load_location_encoder(meta["checkpoint"], device=device)
            except FileNotFoundError:
                print(f"   Checkpoint not found: {meta['checkpoint']}")
                continue
            
            # Compute embeddings
            emb_cpu = compute_embeddings(loc_enc, coords_deg, device=device)
            ...
            # Use consistent naming with other scripts
