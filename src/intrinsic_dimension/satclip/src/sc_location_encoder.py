...
def get_temporal_encoding(name):
    if name == "clay":
        return TE.ClayTemporalEncoding()
...
class TemporalLocationEncoder(nn.Module):
    def __init__(self, posenc, tempenc, nnet):
        super().__init__()
        self.posenc = posenc
    ...
    def forward(self, x, ts):
        #print(torch.isnan(x).any())
        x = self.posenc(x)
        ts  = self.tempenc(ts).to(x.device)
        x = torch.cat((x, ts), 1)
        #print(torch.isnan(x).any())
        return self.nnet(x)