...
from datamodules.transforms import get_rcf_s2_train_transform, get_pretrained_s2_train_transform, get_s2_train_transform
...
class SatCLIP(nn.Module):
 ...
 #          print('using RCF')
            ...
            #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            #self.visual.layer4[1].requires_grad_(True)
            self.visual.layer4[2].requires_grad_(True)
            #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            self.visual.fc.requires_grad_(True)
            ...
            self.visual.requires_grad_(False)
            #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            self.visual.blocks[10].requires_grad_(True)
            self.visual.blocks[11].requires_grad_(True)
            #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def get_dist(self, c):
        maxdist = 12436
        tree = libpysal.cg.KDTree(c.detach().cpu().numpy(), distance_metric='Arc', radius=libpysal.cg.RADIUS_EARTH_MILES)
        sdm = tree.sparse_distance_matrix(tree, maxdist) 
        dist = sdm.toarray()
        dist = torch.tensor(dist,dtype=torch.float32) / maxdist
        return dist
    ...
    def forward(self, image, coords):
        ...
        # get distance
        if self.dist_weight:
            dist = self.get_dist(coords).to(location_features.device)
        ...
        if self.dist_weight:
            logits_per_image = logits_per_image * dist
