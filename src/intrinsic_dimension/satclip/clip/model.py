...
from datamodules.transforms import get_rcf_s2_train_transform
import rcf_linear_batched
...
class GeoCLIP(nn.Module):
    def __init__(self,
                 ...
                 capacity: int=256,
                 rcf_empirical_data_dir: Optional[str] = None, 
                 rcf_seed: int=0,
                 rcf_empirical_transform: str='pretrained',
                 *args,
                 **kwargs
                 ):
        super().__init__()
        ...
        if vision_layers == "rcf_empirical" and rcf_empirical_data_dir is None:
            raise ValueError("rcf_empirical_data_dir must be provided when model is 'rcf_empirical'")
 ...
 #           print('using modified resnet')
        ...
        elif vision_layers == 'rcf_empirical':
 #          print('using RCF')
            if rcf_empirical_transform=='pretrained':
                rcf_transform = get_pretrained_s2_train_transform()
            elif rcf_empirical_transform=='default':
                rcf_transform = get_s2_train_transform()
            elif rcf_empirical_transform=='rcf_transform':
                rcf_transform = get_rcf_s2_train_transform()
            
            rcf_empirical_dataset = S2Geo(root=rcf_empirical_data_dir,
                                          transform=rcf_transform,)
            self.visual = rcf_linear_batched.RCF(in_channels=in_channels,
                                              features=vision_width, 
                                          out_dims=embed_dim, 
                                              mode='empirical',
                                              dataset=rcf_empirical_dataset,
                                              kernel_size=3,
                                              seed=rcf_seed,
                                              patches_per_batch=32)
            self.visual.linear.requires_grad_(True)
            
        elif vision_layers == 'rcf_gaussian':
 #          print('using RCF')
            self.visual = rcf_linear_batched.RCF(in_channels=in_channels,
                                              features=vision_width, 
                                              out_dims=embed_dim, 
            ...
            #self.visual.layer4[1].requires_grad_(True)
    ...
    def initialize_parameters(self):
        ...
        # nn.innit.normal_(self.location.ffn.weight, std=0.02)
    ...
    @property
    def dtype(self):
        if isinstance(self.visual, torchgeo.models.RCF) or isinstance(self.visual, rcf_linear_batched.RCF):
            return self.visual.weights.dtype
...
if __name__=="__main__":
    model_geo_grid = GeoCLIP(
        embed_dim=512,
        in_channels=4, image_resolution=224, vision_layers=12, vision_width=768, vision_patch_size=32, # Image encoder
        le_type='grid', harmonics_calculation="analytic", frequency_num=16, max_radius=0.01, min_radius=0.00001, legendre_polys=10, sh_embedding_dims=16, pe_type='siren', capacity=256, num_hidden_layers=2 # Location encoder
    )
    model_geo_theory = GeoCLIP(
        embed_dim=512,
        in_channels=4, image_resolution=224, vision_layers=12, vision_width=768, vision_patch_size=32, # Image encoder
        le_type='theory', harmonics_calculation="analytic", frequency_num=16, max_radius=0.01, min_radius=0.00001, legendre_polys=10, sh_embedding_dims=16, pe_type='siren', capacity=256, num_hidden_layers=2 # Location encoder
    )
    model_geo_spherical = GeoCLIP(
        embed_dim=512,
        in_channels=4, image_resolution=224, vision_layers=12, vision_width=768, vision_patch_size=32, # Image encoder
        le_type='sphericalharmonics', harmonics_calculation="analytic", frequency_num=16, max_radius=0.01, min_radius=0.00001, legendre_polys=10, sh_embedding_dims=16, pe_type='siren', capacity=256, num_hidden_layers=2 # Location encoder
    )

    img_batch = torch.randn(32, 4, 224, 224) # Represents a batch of 32 images with 4 channels
    loc_batch = torch.randn(32, 2) # Represents the corresponding 32 locations (lon/lat)

    with torch.no_grad():
        logits_per_image, logits_per_coord = model_geo_grid(img_batch, loc_batch)
        probs = logits_per_image.softmax(dim=-1).detach().cpu().numpy()
        print(probs.shape)

        logits_per_image, logits_per_coord = model_geo_theory(img_batch, loc_batch)
        probs = logits_per_image.softmax(dim=-1).detach().cpu().numpy()
        print(probs.shape)

        logits_per_image, logits_per_coord = model_geo_spherical(img_batch, loc_batch)
        probs = logits_per_image.softmax(dim=-1).detach().cpu().numpy()
        print(probs.shape)