...
"""Modified implementation of a random convolutional feature projection model from
https://github.com/microsoft/torchgeo/blob/main/torchgeo/models/rcf.py where this model
allows batching by number of features to be more memory efficient."""
...
class RCF(Module):
    ...
    * https://www.nature.com/articles/s41467-021-24638-z
    ...
    def __init__(
        ...
        patches_per_batch: Optional[int] = None,
    ) -> None:
        ...
        if patches_per_batch is None:
            patches_per_batch = num_patches
        
        self.patches_per_batch = patches_per_batch
        self.num_patch_batches = int(np.ceil(num_patches/patches_per_batch))
        ...
        print('number of unique patches: ', len(self.weights.unique(dim=0)))  
        self.linear = torch.nn.Linear(features, out_dims, bias=True)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x1_patches = []
        bs = self.patches_per_batch
        for i in range(self.num_patch_batches):
            weights_batch = self.weights[i*bs: (i+1)*bs]
            biases_batch = self.biases[i*bs: (i+1)*bs]
            x1a_this = F.relu(
                F.conv2d(x, weights_batch, bias=biases_batch, stride=1, padding=0),
                inplace=True,
            )
            x1a_this = F.adaptive_avg_pool2d(x1a_this, (1, 1)).squeeze()
            x1_patches.append(x1a_this)
        ...
        for i in range(self.num_patch_batches):
            ...
            x1b_this = F.adaptive_avg_pool2d(x1b_this, (1, 1)).squeeze()
            x1_patches.append(x1b_this)
            
        if len(x1a_this.shape) == 1:  # case where we passed a single input
            output = self.linear(torch.cat(x1_patches, dim=0))
            return output
        else:  # case where we passed a batch of > 1 inputs
            assert len(x1a_this.shape) == 2
            output = self.linear(torch.cat(x1_patches, dim=1))
            return output